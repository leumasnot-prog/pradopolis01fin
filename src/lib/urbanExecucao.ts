// Leitura e agregação server-side dos relatórios de execução orçamentária de Serviços Urbanos.
//
// Fontes (pasta /data, exportadas do sistema Fiorilli):
//  - ex-orc-urban2025.csv / ex-orc-urban2026.csv  → empenhos do setor de Serviços Urbanos
//
...
export interface UrbanExecucaoData {
  geradoEm: string;
  resumo: {
  ...
  limite: {
    ...
    minimo2026: number; // meta sugerida do setor (80%)
...
let cache: UrbanExecucaoData | null = null;

export function getUrbanExecucao(force = false): UrbanExecucaoData {
  if (cache && !force) return cache;

  const a2025 = aggregateYear("ex-orc-urban2025.csv");
  const a2026 = aggregateYear("ex-orc-urban2026.csv");
  ...
  cache = {
    ...
    limite: {
      ...
      minimo2026: empenhadoTotal * 0.8, // meta sugerida do setor (80%)
      percentualMinimo: 80,
