// Leitura e agregação server-side dos relatórios de execução orçamentária da Promoção Social.
//
// Fontes (pasta /data, exportadas do sistema Fiorilli):
//  - ex-orc-social2025.csv / ex-orc-social2026.csv  → empenhos da Promoção Social
//
...
export interface SocialExecucaoData {
  geradoEm: string;
  resumo: {
  ...
  limite: {
    ...
    minimo2026: number; // total liquidado 2026 (usado como valor do gauge)
    ...
    atingiu: boolean; // sempre true ou baseado em taxa > 80%? Vamos deixar true se >= 80% (meta de execução)
...
let cache: SocialExecucaoData | null = null;

export function getSocialExecucao(force = false): SocialExecucaoData {
  if (cache && !force) return cache;

  const a2025 = aggregateYear("ex-orc-social2025.csv");
  const a2026 = aggregateYear("ex-orc-social2026.csv");
  ...
  cache = {
    ...
    limite: {
      ...
      minimo2026: empenhadoTotal * 0.8, // Vamos colocar uma meta sugerida de 80% de execução
      percentualMinimo: 80,
