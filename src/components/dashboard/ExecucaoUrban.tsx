...
// Execução do Orçamento de Serviços Urbanos.
// Tela dedicada ao setor de Serviços Urbanos: cumprimento da meta sugerida de 80% de execução,
// comparativo anual de execução (2025 × 2026, até o mês fechado de Maio/05) por fonte de
// recurso e por função/subfunção, e os contratos/despesas fixas do
// Departamento Municipal de Planejamento Urbano, Obras e Saneamento.
...
const SETOR_URBAN = "DEPARTAMENTO MUNICIPAL DE PLANEJAMENTO URBANO, OBRAS, SANEAM";
...
export function ExecucaoUrban() {
  const prefersReducedMotion = useReducedMotion();

  const [data, setData] = useState<UrbanExecucaoData | null>(null);
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    fetch("/api/urban/execucao")
      .then((r) => {
        if (!r.ok) throw new Error("Falha ao carregar dados");
        return r.json();
      })
      .then((d: UrbanExecucaoData) => {
        if (active) {
          setData(d);
          setError("");
        }
      })
      .catch(() => active && setError("Não foi possível processar os relatórios de execução de Serviços Urbanos."))
  ...
  // Contratos do Departamento Municipal de Planejamento Urbano, Obras e Saneamento
  const urbanContracts = useMemo(() => {
    return despesasFixasData.contratos
      .filter((c) => c.setor === SETOR_URBAN)
      .sort((a, b) => b.valor_anual - a.valor_anual);
  }, []);

  const filteredContracts = useMemo(() => {
    const q = contractSearch.toLowerCase();
    return urbanContracts.filter(
      (c) =>
    ...
    return (
      <div className="w-full h-[70vh] flex items-center justify-center px-6">
        <div className="flex flex-col items-center gap-3 text-ink-2">
          ...
          <span className="text-sm font-semibold">Processando relatórios de execução de Serviços Urbanos…</span>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <SectionHeader
          title="Execução do Orçamento — Serviços Urbanos"
          subtitle="Acompanhamento orçamentário dos Serviços Urbanos e Infraestrutura, comparativo anual da execução (2025 × 2026) e contratos fixos do setor"
          badge={
          ...
          <div className="flex flex-col items-center justify-center py-4 relative">
            <div className="relative w-44 h-44 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90">
                <defs>
                  <linearGradient id="gaugeGradientUrban" x1="0" y1="0" x2="1" y2="1">
            ...
            {topSubfuncoes.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topSubfuncoes} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  <defs>
                    <linearGradient id="gradientLiqUrban" x1="0" y1="0" x2="1" y2="0">
                  ...
                  />
                  <Bar dataKey="Liquidado" fill="url(#gradientLiqUrban)" radius={[0, 4, 4, 0]} barSize={14} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
      ...
      {/* Contratos e despesas fixas do setor */}
      <motion.div variants={itemVariants}>
        <Card className="p-6 shadow-sm hover:shadow-md transition-all duration-300">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-display text-lg font-bold text-ink tracking-tight">
                  Contratos e Despesas Fixas — Serviços Urbanos
                </h4>
                <p className="text-xs font-medium text-ink-2">
                  Planejamento Urbano, Obras e Saneamento ·{" "}
