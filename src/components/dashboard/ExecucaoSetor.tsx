...
// Tela GENÉRICA de Execução Setorial.
// Porta o padrão visual de ExecucaoUrban, mas parametrizada por um SetorConfig: serve
// para qualquer setor cuja API exponha o mesmo shape (GET /api/setor/execucao?funcao=NN).
// Mantém KPIs (5 StatCards), gauge de eficiência, comparativo 2025×2026, principais
// subfunções, tabela comparativa alternável (fonte/função) e, quando aplicável, o bloco
// de contratos/despesas fixas do setor (filtrado por config.setorContratoNome).
...
export function ExecucaoSetor({ config }: { config: SetorConfig }) {
  const prefersReducedMotion = useReducedMotion();
  const Icon = config.icon;

  // Ids de gradiente únicos por slug — múltiplas telas podem coexistir no DOM
  // entre transições e ids duplicados de SVG colidiriam.
  const gradId2025 = `grad2025-${config.slug}`;
  const gradId2026 = `grad2026-${config.slug}`;
  const gradIdLiq = `gradLiq-${config.slug}`;

  const [data, setData] = useState<SetorExecucaoData | null>(null);
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    fetch(`/api/setor/execucao?funcao=${config.funcao}`)
      .then((r) => {
        if (!r.ok) throw new Error("Falha ao carregar dados");
        return r.json();
      })
      .then((d: SetorExecucaoData) => {
        if (active) {
          setData(d);
          setError("");
        }
      })
      .catch(() => active && setError(`Não foi possível processar os relatórios de execução de ${config.label}.`))
      .finally(() => active && setLoading(false));
    return () => {
      active = false;
    };
  }, [config.funcao, config.label]);
  ...
  // Contratos do setor (somente quando há um nome de setor configurado).
  const setorContracts = useMemo(() => {
    if (!config.setorContratoNome) return [];
    return despesasFixasData.contratos
      .filter((c) => c.setor === config.setorContratoNome)
      .sort((a, b) => b.valor_anual - a.valor_anual);
  }, [config.setorContratoNome]);

  const filteredContracts = useMemo(() => {
    const q = contractSearch.toLowerCase();
    return setorContracts.filter(
      (c) =>
  ...
  // O bloco de contratos sempre é exibido para o setor (mesmo que a tabela esteja vazia)
  const showContracts = !!config.setorContratoNome;
    ...
    return (
      <div className="w-full h-[70vh] flex items-center justify-center px-6">
        <div className="flex flex-col items-center gap-3 text-ink-2">
          ...
          <span className="text-sm font-semibold">Processando relatórios de execução de {config.label}…</span>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <SectionHeader
          title={`Execução do Orçamento — ${config.label}`}
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="grid grid-cols-2 gap-4 xl:grid-cols-1">
          <StatCard
            ...
            subtitle={`Meta sugerida: ${formatPercent(limite.percentualMinimo)}`}
            icon={ShieldCheck}
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-6 print:grid-cols-12 print:gap-4">
        {/* Gauge: Eficiência de Execução ou Limite Constitucional (para Educação) */}
            ...
            <>
              <div>
                ...
                <p className="text-xs font-medium text-ink-2 mt-0.5">
                  Liquidado da função Educação sobre a receita de impostos e transferências
                </p>
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-4 relative">
                <div className="relative w-44 h-44 flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90">
                    <defs>
                      <linearGradient id="gauge-educacao-const" x1="0" y1="0" x2="1" y2="1">
                    ...
                    <motion.circle
                      ...
                      cy="88"
                      r="76"
                      stroke="url(#gauge-educacao-const)"
                      strokeWidth="13"
                      fill="transparent"
                  ...
                  <div className="absolute flex flex-col items-center justify-center">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-[0.12em] mt-1.5 text-pos">
                      Limite atingido
                    </span>
                    <span className="text-[10px] font-semibold text-muted mt-0.5">mínimo 25%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-surface-2 rounded-lg p-4 border border-line text-center space-y-1 mt-2">
                <span className="text-xs font-medium text-ink-2 block">
                  Aplicado <strong className="font-mono tabular text-ink">R$ 19.552.358,60</strong> de uma base de <strong className="font-mono tabular text-ink">R$ 46.627.977,12</strong>.
                </span>
                <span className="text-[11px] font-medium text-muted block">
                  Limite de 25%: <strong className="font-mono tabular text-warn">R$ 11.656.994,28</strong>
                  {" · "}
                  Acima do limite em <strong className="font-mono tabular text-pos">R$ 7.895.364,32</strong>
                </span>
              </div>
            </>
          ) : (
            <>
              ...
              <div className="flex flex-col items-center justify-center py-4 relative">
                <div className="relative w-44 h-44 flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90">
                    <defs>
                      <linearGradient id={config.gaugeGradientId} x1="0" y1="0" x2="1" y2="1">
                    ...
                    <motion.circle
                      ...
                      cy="88"
                      r="76"
                      stroke={`url(#${config.gaugeGradientId})`}
                      strokeWidth="13"
                      fill="transparent"
                      strokeDasharray={RING_LEN}
        ...
        <Card className="lg:col-span-4 p-6 flex flex-col justify-between min-h-[390px] shadow-sm hover:shadow-md transition-all duration-300 print:col-span-4 print:break-inside-avoid">
          <div>
            <span className="text-[11px] font-semibold text-muted uppercase tracking-[0.14em] block">
              Comparativo anual · mesmo período acumulado
            </span>
          ...
          <div className="w-full h-64 mt-4 relative">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={comparativoBarData} margin={{ top: 10, right: 10, left: 10, bottom: 5 }} barGap={4}>
                <defs>
                  <linearGradient id={gradId2025} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#A8B3C4" stopOpacity={0.9} />
                    <stop offset="100%" stopColor="#D7DCE3" stopOpacity={0.3} />
                  </linearGradient>
                  <linearGradient id={gradId2026} x1="0" y1="0" x2="0" y2="1">
                ...
                <Bar dataKey="2025" fill={`url(#${gradId2025})`} radius={[4, 4, 0, 0]} isAnimationActive={!prefersReducedMotion} />
                <Bar dataKey="2026" fill={`url(#${gradId2026})`} radius={[4, 4, 0, 0]} isAnimationActive={!prefersReducedMotion} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
            ...
            {topSubfuncoes.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topSubfuncoes} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  <defs>
                    <linearGradient id={gradIdLiq} x1="0" y1="0" x2="1" y2="0">
                  ...
                  />
                  <Bar dataKey="Liquidado" fill={`url(#${gradIdLiq})`} radius={[0, 4, 4, 0]} barSize={14} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-medium text-ink-2">
                  Agrupado por {groupMode === "fonte" ? "fonte de recurso" : "função / subfunção"} (mesmo período acumulado) ·{" "}
      ...
      {/* Contratos e despesas fixas do setor (apenas se configurado e houver registros) */}
      {showContracts && (
        <motion.div variants={itemVariants}>
          <Card className="p-6 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-display text-lg font-bold text-ink tracking-tight">
                    Contratos e Despesas Fixas — {config.label}
                  </h4>
                  <p className="text-xs font-medium text-ink-2">
                    Compromissos contratuais do setor ·{" "}
